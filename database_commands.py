...
class DataBase:
    """
    Класс, инкапсулирующий в себя все функции,
    связанные с манипуляциями с базой данных
    """
    def __init__(self):
        self.conn = psycopg2.connect(database='eng_cards_db',
                                     user = 'postgres',
    ...
    def add_user(self, user_id):
        ...
        if not DataBase.check_user(self, self.user_id):
            with self.conn:
                self.cur.execute("""INSERT INTO users (name)
                                          VALUES (%s);""", (user1,))
                self.conn.commit()
        else:
    ...
    def create_user_table(self, user_id):
        """
        Функция для создания отельной таблицы
        ...
        if DataBase.check_user(self, self.user_id):
            with self.conn:
                self.cur.execute(sql.SQL(
                    ...
                    (SELECT eng, ru FROM zero);""").format
                                 (table=sql.Identifier(user1)))
                try:
                    self.cur.execute(sql.SQL("""ALTER TABLE {table}
                                    ADD COLUMN id SERIAL PRIMARY KEY,
                                    ADD CONSTRAINT eng_only
                                    CHECK(eng ~*'[a-z]+'),
                                    ADD CONSTRAINT {en_constraint}
                                    UNIQUE (eng),
                                    ALTER COLUMN eng SET NOT NULL,
                                    ADD CONSTRAINT ru_only
                                    CHECK(ru ~*'[а-яё]+'),
                                    ALTER COLUMN ru SET NOT NULL;""").
                                     format(table=sql.Identifier(user1),
    ...
    def check_word(self, new_word, user_id):
        ...
        user1 = 'user_' + str(self.user_id)
        with self.conn:
            self.cur.execute(sql.SQL("""SELECT FROM {table}
            WHERE eng = %s;""").
                             format(table=sql.Identifier(user1),), (new_word,))
            self.conn.commit()
            return bool(self.cur.fetchall())
    ...
    def add_word(self, new_word, translate, user_id):
        ...
        if not (DataBase.check_word(self, self.new_word, self.user_id) and
                DataBase.check_word(self, self.translate, self.user_id)):
            with self.conn:
    ...
    def get_eng_words(self, user_id):
        """
        Функция для получения списка доступных слов,
        используется вспомогательно для преобразования в список данных
        из БД
        """
        self.user_id = user_id
        user1 = 'user_' + str(self.user_id)
        if DataBase.check_user(self, self.user_id):
            with self.conn:
                self.cur.execute(sql.SQL("""SELECT eng FROM {table} """)
                                 .format(table=sql.Identifier(user1)))
                eng_list = []
    ...
    def get_words_count(self, user_id):
        """
        Функция, необходимая для подсчёта слов
        у пользователя. Нужна для защиты от "хитрого"
        пользователя
        """
        self.user_id = user_id
        user1 = 'user_' + str(self.user_id)
        if DataBase.check_user(self, self.user_id):
            with self.conn:
                self.cur.execute(sql.SQL("""SELECT COUNT(*) FROM {table} """)
                                 .format(table=sql.Identifier(user1)))
                result = self.cur.fetchone()[0]
                return result
